...
export default function Team() {
  ...
  const teamMembers = [
    {
      name: "Machochi Chekanabo",
      role: "Founder & Director",
      initials: "MC",
      hasImage: true
    },
    {
      name: "Zacharia Mwanuke",
      role: "CEO",
      initials: "ZM",
      hasImage: true
    },
    {
      name: "Peter Joasi Idd",
      role: "Operations Manager",
      initials: "PJ"
    },
    {
      name: "Zakaria Tundula Ernest",
      role: "Technician",
      initials: "ZT"
    }
  ];
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <nav className="sticky top-0 z-50 bg-white border-b border-gray-200 shadow-sm">
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <div className="flex gap-8 text-sm font-medium">
            ...
            <button onClick={() => setLocation("/team")} className="text-blue-700 hover:text-blue-900">Team</button>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            {teamMembers.map((member, index) => {
              const imageMap: {[key: string]: string} = {
                "Machochi Chekanabo": "/machochi.jpg",
                "Zacharia Mwanuke": "/zacharia.webp"
              };
              return (
              <Card key={index} className="p-6 text-center hover:shadow-lg transition-shadow">
                {member.hasImage ? (
                  <img 
                    src={imageMap[member.name] || "/machochi.jpg"} 
                    alt={member.name}
                    className="w-32 h-32 rounded-full object-cover mx-auto mb-4 border-4 border-blue-100"
                  />
                ) : (
                  <div className="w-32 h-32 rounded-full bg-blue-100 flex items-center justify-center mx-auto mb-4 border-4 border-blue-100">
                    <span className="text-3xl font-bold text-blue-600">{member.initials}</span>
                  </div>
                )}
      ...
      <section className="py-16 px-4 bg-gray-50">
        <div className="container mx-auto max-w-4xl">
          <h2 className="text-3xl font-bold text-blue-900 mb-8 text-center">Our Team Values</h2>
          <div className="grid md:grid-cols-3 gap-6">
            <Card className="p-6 border-t-4 border-t-blue-600">
              <h3 className="text-xl font-bold text-blue-900 mb-3">Expertise</h3>
              <p className="text-gray-700">Deep knowledge and experience in aquaculture operations and management.</p>
            </Card>
            <Card className="p-6 border-t-4 border-t-green-600">
              <h3 className="text-xl font-bold text-blue-900 mb-3">Dedication</h3>
              <p className="text-gray-700">Committed to excellence and continuous improvement in all operations.</p>
            </Card>
            <Card className="p-6 border-t-4 border-t-blue-600">
              <h3 className="text-xl font-bold text-blue-900 mb-3">Innovation</h3>
              <p className="text-gray-700">Embracing modern technology and data-driven approaches for better results.</p>
            </Card>
          </div>
        </div>
      </section>
